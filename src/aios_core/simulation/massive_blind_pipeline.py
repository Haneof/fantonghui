...
class MegaBlindPipeline:
            ...
            for obs in stream:
                if obs.kind == "photo":
                    ...
                    if "HR=" in obs.payload:
                        ...
                        hr_going.append(
                            WaveformSample(ts_ms=obs.minute_of_day * 60_000, value=hr)
                        )
        report.retained_bytes = cleaner.raw_byte_sink.retained_byte_count
        if hr_going:
            comp = AdaptiveTimeSeriesCompressor.compress(hr_going)
            report.hr_baseline_delta = (
                statistics.fmean(s.value for s in hr_going) - hr_baseline
            )
    ...
    def run_stage2(
        self,
        bench: MassiveLifeDataBench,
        index: CJKTopologicalInvertedIndex,
        report: StageTwoReport,
    ) -> dict[int, list[str]]:
        ...
        for persona_id, kind, stream in bench.iter_domains():
            for obs in stream:
                ...
                if obs.kind in ("vitals_hr", "financial", "gps"):
                    index.index_entity_text(
                        obs.obs_id, obs.payload, obs.minute_of_day * 60_000_000_000
                    )
    ...
    def run_stage5(
            ...
            for obs in stream:
                if obs.kind == "financial":
                    fact = SealedFactBytes(
                        ...
                        bytes_sha256=FactRegistry.digest_of(
                            obs.payload.encode("utf-8")
                        ),
        ...
        except RegistryImmutableViolationError:
    ...
    def run_stage6(self, ledger: EvidenceLedger, report: StageSixReport) -> None:
        ...
        refs = {
            ...
            "gift_2024_footbath_idle": ObjectRef(
                object_id="gift_2024_footbath_idle", revision=1
            ),
            "gift_2025_massage_chair": ObjectRef(
                object_id="gift_2025_massage_chair", revision=1
            ),
            "health_2026_knee_cold": ObjectRef(
                object_id="health_2026_knee_cold", revision=1
            ),
            "court_judgment_2024": ObjectRef(
                object_id="court_judgment_2024", revision=1
            ),
            "wechat_loan_unpaid_2024": ObjectRef(
                object_id="wechat_loan_unpaid_2024", revision=1
            ),
            "work_all_nighter_chain": ObjectRef(
                object_id="work_all_nighter_chain", revision=1
            ),
            "pvc_holter_report": ObjectRef(object_id="pvc_holter_report", revision=1),
        }
        report.advices_issued += 1 if mom.advise(refs) else 0
        report.advices_issued += (
            1 if fraud.advise(refs, requested_amount_cny=200_000) else 0
        )
        report.advices_issued += (
            1
            if health.advise(refs, consecutive_all_nighters=4, pvc_burden_per_1000=6.8)
            else 0
        )
    ...
    def run_stage8(
        self, archive: ConversationObservationArchive, report: StageEightReport
    ) -> None:
        ...
        for i in range(10):
            ...
            archive.append_turn(
                ConversationTurn(
                    turn_id=f"turn_{seq}",
                    sequence_no=seq,
                    occurred_at=datetime(2026, 9, 16, 12, i, tzinfo=UTC),
                    user_text=text,
                    assistant_text=text,
                )
            )
    ...
    def run_full(self, bench: MassiveLifeDataBench) -> FullPipelineReport:
        ...
        report.memory_peak_rss_mb = (
            resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024.0
        )
        return report

    def pace_benchmark(
        self, bench: MassiveLifeDataBench, *, chunk_size: int = 5_000
    ) -> list[float]:
...
def render_report_markdown(report: FullPipelineReport) -> str:
    ...
    rows = [
        (
            "S1 端侧摄入",
            f"帧->{report.stage1.raw_frames} 采纳->{report.stage1.acceptable_frames} 留存字节->{report.stage1.retained_bytes}",
        ),
        (
            "S2 金字塔",
            f"日->{report.stage2.days_crystallized} 周->{report.stage2.weeks_crystallized} 月->{report.stage2.months_crystallized} 指针失败->{report.stage2.pointer_resolution_failures}",
        ),
        (
            "S3 共现召回",
            f"查询->{report.stage3.co_searches} 事件链路->{report.stage3.event_lifecycle_completed}",
        ),
        (
            "S4 认知导数",
            f"Burnout熔断->{report.stage4.dimensions_burnout} 信用闸门->{report.stage4.dimensions_credit} 拐点->{report.stage4.inflection_fired}",
        ),
        (
            "S5 单跳隔离",
            f"封存事实->{report.stage5.sealed_facts} 注解->{report.stage5.annotations_mounted} 篡改封锁->{report.stage5.tamper_attempts_blocked}",
        ),
        (
            "S6 决策推演",
            f"建议->{report.stage6.advices_issued} 证指->{report.stage6.evidences_cited} 套话拒->{report.stage6.generic_fluffs_blocked}",
        ),
        (
            "S7 沟通博弈",
            f"反思->{report.stage7.comm_experiences} 反谄媚->{report.stage7.anti_flattery_holds} 反师爷->{report.stage7.no_teacher_speech} UI净->{report.stage7.blackbox_ui_clean}",
        ),
        (
            "S8 驾驶舱",
            f"P0->{report.stage8.hard_bypass_p0_ms:.3f}ms LLM->{report.stage8.hard_bypass_llm_calls} 隐藏tok->{report.stage8.hidden_tokens} 轮窗->{report.stage8.active_window_turns} 三句->{report.stage8.u_turns_within_3_sentences}/10",
        ),
        (
            "总耗费",
            f"{report.wall_time_seconds:.2f}s 内存峰值->{report.memory_peak_rss_mb:.1f}MB",
        ),
    ]
    s.append("| 门 | 量纲 | 结果 |")
    s.append("|---|---|---|")
