...
10. 策略性数值未登记 `code/policies_v0.json` 且无豁免理由 = 打回（执行器 `policy_scan.py --strict`，有退出码，不是道德口号）；
11. 文档里只给数值、不给"默认值 + [地板,天花板] + 谁能改 + 改了会怎样"四件套的，按未完成处理（指挥官纪律，12 号文档 Part A）。
...
| 22 | **参数注册表已建但未接线**：23 个旋钮有户口，仅 1 个被代码读取（判分器） | 12 号文档 Part A；NEXT_TASK 任务 J | **P0** | `python3 code/policy_scan.py` → `已被代码引用 1`；`evolutiond.py:28-29`、`hublinkd.py:21-22` 仍是硬编码常量 |

新增红线 R1-R6（见 `NEXT_TASK.md` 第三/四批），第四批按**可变性三层**重标：R1/R2/R4 = T0 结构不变量（App 禁止自算档位、`ABSENT ≠ 0`、换 rubric 必须打断点），R3 = T0 原则 + T1 参数（可证伪是结构，"几次算证伪"是旋钮），R5/R6 = T1/T2 的参数纪律（未登记即打回、`safety_linked` 项 AI 无权放宽）。

---

## 十一、同日第四批：把"没有任何东西写死"变成可执行的东西

### 指挥官裁定（本批全部工作的依据）

> **「我们的开发过程，没有任何东西是写死的，哪怕你标记了，也可能会改。」**
> 采集策略：**按时间段总结一次**；触发频率/采集密度 **AI 有权调，或设置里调**；**除了安全那些不能乱调**。
> 提出的真问题：**声音对话是被动采集，身体指数是主动采集，可它每秒都有数据——难道让传感器不停地检测吗？**

### 我上一批的措辞错误（已改，记录在案）

我把 `≤6/≤40`、`≤0.5`、"红线 R1-R4"写成了铁律。**默认值伪装成物理定律，是文档层面的写死**。第四批的修法不是删数字，是给每个数字配上"谁有权改 + 改了会怎样 + 怎么回滚"。

### 本批产出

| 交付 | 内容 | 状态 |
|---|---|---|
| `code/policies_v0.json` | **23 个旋钮**：采样频率、占空比提频/防抖/冷却、聚合窗口、保留期、总结节奏、平线窗口、置信度封顶、维度上限、拾音窗口、AI 会话预算、AI 自调夹逼区间、hublinkd 攒批；每根带 `default/floor/ceiling/owner/ai_may_relax/safety_linked/hot_effect/audit/rationale` + 4 条豁免 | 生效（户口建好） |
| `code/policy_scan.py` | RULE-1..6 机械校验，`--json/--strict`，有退出码 | **首跑即抓到主线代码 3 处真写死**：`evolutiond.THRESHOLD_LOW/HIGH=0.05/0.95`（AI 自调的夹逼区间本身被写死）、`hublinkd.BATCH_WINDOW/BATCH_SIZE`（"829 条/秒"那个性能结论的测试配置） |
| `docs/12_POLICY_AND_SAMPLING_V0.md` | Part A 可变性三层与变更协议；Part B 四道闸门 + 三层频率 + 提频须 Trigger 授权 + 漏报风险必须声明 | 契约 v0 |
| `bench/check_teaching_fit.py` | 判分器**改为读注册表**取置信度封顶；夹具与注册表不一致 → 报 `K9_夹具与注册表口径不一致`（已实测：改夹具→报警，还原→消失） | 实跑通过 |

### 传感器问题的答案（一句话）

**不该每秒采，更不该每秒进曲线**：硬件侧先事件化（25Hz→每天几百条边沿事件）→ 占空比（平时 5 分钟一次，Trigger 活跃窗口提到 10 秒，冷却回落）→ 越线才报 → 曲线只存窗口统计（5 分钟一点，90 天 = 2.6 万点，AI 一次读完，原始流 48 小时后只留聚合）。
关键约束两条：**提频必须由 Trigger 授权**（不许 AI 凭好奇常开）；**省电不得影响安全**（`safety.fall_detect_hz` 地板=天花板=25，AI 无权放宽；漏报风险必须在维度注册时用 `miss_profile` 写出来，不假装 5 分钟采样看得见 20 秒的心动过速）。

### 未完成 / 待拍板

- 看板 #22：注册表**尚未被服务读取**——接线前"参数可调"这句话是假的。任务 J。
- 待拍板（我未自行决定）：可变性三层是否上升为 **V1.5 修宪条款**。本批只写进 `docs/01 §2 硬规则` 与 12 号契约文档，**未改 V1.4 正文**（§七-4）。
- 教育竖切三项（优先级 / 首个数据源 / Windows 侧谁跑）指挥官仍未答，沿用第四批自决：不插队演示型竖切、先模拟器、Windows 挂账。
