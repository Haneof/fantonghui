...
| POLICY-RUNTIME | `governance/runtime_policy.json` | 1.2.0 | （待回填） | **CURRENT**（1.2.0=两条工作线的 v1.1.0 合并：ADJ-001~012 全量对齐 + retention_ttl 入法；27 域；宪法→法律→判决之法律层） | CONST-v3.0.1 |
| PLAN-R4-B | `AIOS_Core_任务规划与开发任务拆分重构方案_R4.md` | R4.1 | （待回填） | **CURRENT**（R4.1：§2.1 增编号对撞警告横幅；THRESH-BASE 与 M1-019 落地后两轮对齐） | CONST-v3.0.1 |
