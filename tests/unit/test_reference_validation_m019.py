...
class GenericRefHolder(WorldObject):
    object_type: ObjectType = ObjectType.ENTITY
    refs: list[ObjectRef] = []


class GenericSourceHolder(WorldObject):
    object_type: ObjectType = ObjectType.ENTITY
...
def obs(object_id: str, revision: int = 1, *, learned_at: datetime = NOW) -> Observation:
    return Observation(
        object_id=object_id,
        subject_id="user_1",
        revision=revision,
...
def test_m019_01_missing_object_ref_rejected_without_partial_commit(tmp_path):
    ...
    holder = GenericRefHolder(
        ...
        recorded_at=NOW,
        created_by="m0-019-test",
        refs=[ObjectRef(object_id="missing", revision=1)],
    )
...
def test_m019_03_same_transaction_mutual_non_evidence_refs_remain_legal(tmp_path):
    store = SQLiteWorldStore(tmp_path / "world.db")
    a_id = new_object_id(ObjectType.ENTITY)
    b_id = new_object_id(ObjectType.ENTITY)
    a = GenericRefHolder(
        object_id=a_id,
        subject_id="user_1",
        revision=1,
        ...
        recorded_at=NOW,
        created_by="m0-019-test",
        refs=[ObjectRef(object_id=b_id, revision=1)],
    )
    b = GenericRefHolder(
        object_id=b_id,
        subject_id="user_1",
        ...
        recorded_at=NOW,
        created_by="m0-019-test",
        refs=[ObjectRef(object_id=a_id, revision=1)],
    )
...
def test_m019_04_current_revision_objectref_self_reference_rejected(tmp_path):
    ...
    holder = GenericRefHolder(
        ...
        recorded_at=NOW,
        created_by="m0-019-test",
        refs=[ObjectRef(object_id=object_id, revision=1)],
    )
...
def test_m019_06_previous_revision_self_link_is_historical_and_legal(tmp_path):
    ...
    store.commit([first], op(0, "self-history-v1"))

    second = GenericRefHolder(
        object_id=object_id,
        subject_id="user_1",
        revision=2,
        learned_at=NOW,
...
def test_m019_08_pending_future_reference_rejected_by_knowledge_cutoff(tmp_path):
    ...
    holder = GenericRefHolder(
        ...
        recorded_at=older,
        created_by="m0-019-test",
        refs=[ObjectRef(object_id=target_id, revision=1)],
    )
