...
| **13号总工令** | [`TASK_DISPATCH_3YEAR_MASSIVE_LIFE_COGNITIVE_EVOLUTION.md`](./TASK_DISPATCH_3YEAR_MASSIVE_LIFE_COGNITIVE_EVOLUTION.md) | `M5-3YEAR-SYSTEM-ARENA` 三年前瞻·数万人千人千面原生系统级实战（3年人生底层数据入库 × 真实 AIOS 内核演进 × 存储容量测算与系统优化） | `arena/agent-*-3year-*` | `src/aios_core/simulation/massive_life_store_feeder.py` + `storage/sqlite_store.py` |
| ↳ **提示词 A** | [`PROMPT_TASK_CREATOR_3YEAR_RAW_DATA_INGESTION.md`](./PROMPT_TASK_CREATOR_3YEAR_RAW_DATA_INGESTION.md) | 【出题战队专属】千人千面3年多模态底层生活数据（传感器/MIC/照片描述/APP/聊天）直接入库与容量测算 | `arena/agent-*-3year-feeder` | `data/feeders/{batch_id}/` + `data/worlds/{subject_id}.db` |
| ↳ **提示词 B** | [`PROMPT_TASK_SOLVER_AIOS_NATIVE_COGNITIVE_EVOLUTION.md`](./PROMPT_TASK_SOLVER_AIOS_NATIVE_COGNITIVE_EVOLUTION.md) | 【做题战队专属】AIOS 3.0 系统原生大脑运转与认知演化（逐日推算/高阶维度/AI自身世界注册/看板与Token系统优化） | `arena/agent-*-3year-opt` | `src/aios_core/query/search.py` + `cognition/` + `manifest/` |
