...
## 0.1 可校验整合镜像（带来源 SHA-256 锚，机器可验）

> [`AIOS_Constitution_v3.0_FINAL_CONSOLIDATED.md`](./AIOS_Constitution_v3.0_FINAL_CONSOLIDATED.md) —— 2026-09-18 与 §0 同日完成的另一份整合：对同一现行规范栈（宣言书 / 主宪法 / ADJ 裁决集 / R5 / R6 / R4 候选案 / 路线图附录）逐字并入，每编带源文件 SHA-256 锚。`python3 scripts/assemble_constitution_v3_final.py --check` 可机器校验各编与源文件字节级一致。**引用宪法优先 §0；本镜像用于审计、漂移核对与源文追溯。** 源文件一律不修改；如有出入，以源文件为准。
