...
> **配套产物**：
> - `governance/runtime_policy.json` **v1.2.0** —— **机器可读的运行时政策层（本方案的核心新构件，已落盘；v1.1.0 已按 v3.0.1 裁决集 ADJ-001~012 全量对齐）**
> - `tests/policy/test_runtime_policy.py` —— **政策层判决（已落盘并实测通过：130/130，纯标准库，pytest 与独立运行双入口）**
> - `tests/policy/test_policy_gate_regression.py` —— **给判决本身写的回归测试（已落盘并实测通过：242/242 个违宪变异全部被抓住）**
...
> - `tests/policy/test_hash_registry_versioning.py` —— **规范版本注册表的版本化登记纪律（16 条，已实测通过）**：处置 `registry.md` 规则②③的组合死结（已登记文件原本无法合法演进 —— 两条工作线同日各自独立撞上）；采纳位置语义（追加序即时间序），并钉住它的两条边界：归组键必须是 `(spec_id, path)`（否则**当前生效宪基 CONST-v3.0.1 会被静默移出哈希校验**）、末行占位必须判红（否则表尾追加一行占位即可让任意已登记文件退出校验）
> - `tools/governance/hash_registry.py` —— **采纳位置语义的版本追加机制 + 归组键修正 + 末行占位判红 + 修 `--fill` 排版压扁缺陷**（后者是可审计性缺陷：治理表靠 diff 评审，一次只改哈希格的补登若把整行重排，评审者就看不出动了哪一格）
...
│   职责：每次 push 检查系统是否守宪                                      │
│   形态：130 条判决断言 + 242 个违宪变异回归，纯标准库，fail-closed      │
...
> ### ⚠️ 本节模块编号存在未裁决的对撞，引用前必读
>
> **`ADJ-010 §1` 已裁定「AC 模块映射唯一本为【R4 重构方案 §2.1】的 C01~C16 全表」，
> 而它指向的是 `AIOS_Core_任务规划与开发任务拆分重构方案_R4_独立首席架构师版.md`（已在
> `governance/normative_versions/registry.md` 登记为 PLAN-R4），不是本文件。**
>
> 四个编号直接对撞，且**追溯矩阵已按 PLAN-R4 语义落库**
> （`traceability_matrix.csv` 第 6 行 `C02-no_forced_interrupt,...,C16,NotificationReceipt`）：
>
> | ID | **PLAN-R4（已落库，权威）** | 本节（未登记） |
> |---|---|---|
...
> | **C18** | 不存在 | 政策与合规守卫 |
>
> **按 `registry.md` 规则④，本文件未登记，因此本节目前不构成工程基准。
> 按 `ADJ-010 §3`，模块新增/合并/改义必须走一级治理变更 —— 政策层与本文件均无权自裁。**
>
> **我提交的合并提案**（详见 `reviews/R4_policy_vs_ADJ_alignment_2026-09-15.md` §0）：
> 保留 PLAN-R4 的 C15/C16；**本节的四个职能改编号为 C17~C20**
> （仿真评估与纵向守卫→C17、端侧摄入适配→C18、资源治理与预算执行→C19、政策与合规守卫→C20）；
> 本方案的 `M0.1` 并入 ADJ-010 的 `M0'`，`CP1` 并入 `M1.X`。
> 原则：**已落库到追溯矩阵的编号有既成事实的迁移成本优势，未落库的一方改名，社会成本最低。**
>
> **C19/C20 不应因编号冲突而丢弃** —— 本次政策层对齐的全部工作量都落在 C20 的职责范围内。
>
> **自我评估**：我在政策层 v1.0.0 里写 `id_namespace_registry` 专门防止编号漂移，
> 然后在同一周里成了它的第二个案例。教训是：**命名冲突检测器不能只对着旧文档跑，
> 它必须对着所有新文档跑，包括它自己作者写的那一份。**
> 已落成断言 `test_contested_namespaces_are_marked_as_proposals_not_baselines`。
>
> **下文表格中的 C15~C18 请按上述提案读作 C17~C20；C01~C14 与 PLAN-R4 无冲突。**
...
| **M0.1-008** | **`governance/runtime_policy.json` v1.2.0 + `tests/policy/`**（C19/C20 的落地，编号待 ADJ-010 一级治理变更裁决）<br/>**状态：已交付可运行版本 —— 判决门 130/130 通过，变异回归 242/242 全部抓住违宪；已按 v3.0.1 裁决集全量对齐；THRESH-BASE（M0-031）与 M1-019 落地后已完成两轮跨工件对齐；v1.2.0 = 两条工作线各自产出的 v1.1.0 之合并** | **总工亲自** |
