    ...
    def _build_key_events(self) -> None:
        ...
        ev += [
            ...
            ("obs_e2e_partner_bank_flow", datetime(2024, 9, 15, 10, 0, tzinfo=UTC), "finance",
             "银行流水核对：备忘录记载投入 300 万（实质是借贷关系——只投不经营、一年必退出），实际项目账户仅入账 180 万，差额 120 万去向不明。老王口头解释'走线下供应商'，无票据。", "core_evidence"),
            ("obs_e2e_partner_fight", datetime(2025, 5, 20, 23, 40, tzinfo=UTC), "chat",
             "深夜电话撕逼 47 分钟：'我没骗你，是下游国企卡审批！' vs '流水不会说谎，120 万你给个说法。' 通话录音留存，双方情绪峰值心率均超 130。", "core_evidence"),
            ("obs_e2e_partner_delay", datetime(2025, 11, 2, 9, 0, tzinfo=UTC), "finance",
             "首期退出款项（本金 300 万）到期未付，老王书面申请延期 6 个月，称'新投资人过桥资金下周到位，先转这一期，剩下连本带息'（2025-11-02 电话录音），未附任何担保。我重新核对银行流水：项目账户余额已从 180 万降至 4 万，这笔延期实质是新增借贷请求。", "core_evidence"),
