...
**PASS（有条件）**：功能与宪法红线全部满足，全仓 **1122 项测试通过**，治理登记自检绿
（16 行已验证 / 5 行封存）。
...
YEAR（≤ 366 天）。10 年视图目前只能由多个 YEAR 总结并列承载。补 DECADE 尺度会改动
`SCALE_ORDER` 这个法定序，属**一级治理变更**，我没有私自扩展。
**建议**：走治理变更流程，同时确认滑动条 UI 在 1~10 年区间的实际取数方式。
...
```
tests/unit/test_m1_010r_pyramid.py     48 passed   （原 25 + 新增 23）
全仓                                    1122 passed
