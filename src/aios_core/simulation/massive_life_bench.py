...
class MassiveLifeBenchGenerator:
    ...
    def generate_world_dataset(
        ...
        claim_wang = Claim(
            ...
            support_evidence_set_refs=[ObjectRef(object_id=ev_wang.object_id, revision=1)],
            occurred=TemporalExtent.point(t_wang_court),
            learned_at=t_wang_court,
            recorded_at=t_wang_court,
            created_by="sim_generator",
        ...
        claim_mom = Claim(
            ...
            knowledge_state=KnowledgeState.INFERRED,
            support_evidence_set_refs=[ObjectRef(object_id=ev_mom.object_id, revision=1)],
            occurred=TemporalExtent.point(t_mom_2026),
            learned_at=t_mom_2026,
            recorded_at=t_mom_2026,
            created_by="sim_generator",
        ...
        claim_rom = Claim(
            ...
            knowledge_state=KnowledgeState.INFERRED,
            support_evidence_set_refs=[ObjectRef(object_id=ev_rom.object_id, revision=1)],
            occurred=TemporalExtent.point(t_romance_end),
            learned_at=t_romance_end,
            recorded_at=t_romance_end,
            created_by="sim_generator",
        ...
        ev_health_work = EvidenceSet(
            ...
            knowledge_window=KnowledgeWindow(knowledge_cutoff=datetime(2025, 7, 30, 10, 0, tzinfo=UTC)),
            member_refs=[
                ObjectRef(object_id=o.object_id, revision=1)
                for o in work_obs_list + bio_obs_list
            ],
            selection_method="curated",
            occurred=TemporalExtent(start=datetime(2025, 7, 15, 2, 0, tzinfo=UTC), end=datetime(2025, 7, 29, 9, 0, tzinfo=UTC)),
        ...
        anchor_health_work = EventAnchor(
            ...
            evidence_set_refs=[ObjectRef(object_id=ev_health_work.object_id, revision=1)],
            event_time=TemporalExtent(start=datetime(2025, 7, 15, 2, 0, tzinfo=UTC), end=datetime(2025, 7, 29, 9, 0, tzinfo=UTC)),
            occurred=TemporalExtent(start=datetime(2025, 7, 15, 2, 0, tzinfo=UTC), end=datetime(2025, 7, 29, 9, 0, tzinfo=UTC)),
            learned_at=datetime(2025, 7, 30, 10, 0, tzinfo=UTC),
            recorded_at=datetime(2025, 7, 30, 10, 0, tzinfo=UTC),
            created_by="sim_generator",
        ...
        claim_health_work = Claim(
            ...
            knowledge_state=KnowledgeState.INFERRED,
            support_evidence_set_refs=[ObjectRef(object_id=ev_health_work.object_id, revision=1)],
        ...
        for idx in range(remaining_count):
            ...
            if kind_rand < 0.65:
                ...
                obs = Observation(
                    ...
                    source_kind="biometrics",
                    modality="json",
                    value=json.dumps({"heart_rate": hr, "steps": self.rng.randint(0, 150)}),
...
def populate_massive_world(
    ...
    t0 = datetime.now()
    objects, ent_ids = gen.generate_world_dataset(target_count=target_count)
