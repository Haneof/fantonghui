...
- **A01**：同一基础资料可被多个维度引用，底层事实不复制。
- **A02**：用户世界与 AI 世界使用同一时间和对象机制。
- **A03**：未知实体改名后，旧引用仍能解析。
- **A04**：修正一项底层主张，能找到受影响认知和任务。
- **A05**：查询可重建“某时刻系统当时知道什么”。
- **A06**：重复投递、重启和模型重试不产生重复行动。
- **A07**：无用户提问时能够触发、调查、决策、任务化。
- **A08**：用户接受、执行成功、目标改善三个结果分别记录。
- **A09**：所有关键修改和外部模拟动作具有可回放轨迹。
- **A10**：任何运行接口都不能读到测试隐藏真值。
