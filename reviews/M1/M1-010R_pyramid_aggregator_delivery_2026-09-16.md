...
**PASS**：全仓 **1185 项测试通过**（M1-010R 单文件 **71 项**），治理登记自检绿
（16 行已验证 / 6 行封存）。工单 §2 的两处原本只是"写了常量/写了注释"的要求已实质闭合：

- 「1 秒到 10 年连续无损缩放」→ 取数层已补齐，全量程无死区（§9）；
- 「底层原始事实必须永存」→ 跨进程永存契约已落地，重启后事实一条不少、
  §93 篡改检测跨重启生效（§10）。
...
3. **补 46 项测试**（§4 + §9 + §10），并用 17 个变异测试证明它们不是空转
   （§3 四个 / §9.4 七个 / §10.5 六个）；
4. **补齐滑动条取数层**（§9），闭合 1 年 ~ 10 年段的取数缺口；
5. **落地跨进程永存契约**（§10），使 §25「原始事实永存」在重启后依然成立。
...
**G1 — vault 未持久化（对应 P2）→ 本轮已落地契约，见 §10**
宪法 §25 要求的是**跨进程**永存，而默认 vault 是进程内 dict：月总结生成后日记录确实
一条没删，但一次重启就把十年的事实全带走了。那不是压缩删除，却是同一件事的另一种结果。
本轮以**注入式后端契约**闭合（`VaultBackend`），没有替 storage 层选定引擎。
剩余待决项（需 storage 层承接）：落盘时机（写入即落盘 vs 批量）、写入放大预算、
以及规模再上一个量级时按 id 惰性装载（给契约补 `get(event_id)`）以避免全量水合。
...
## §10 跨进程永存：让 §25「原始事实永存」在重启后依然成立

### 10.1 默认配置下这条红线其实不成立

原实现的 docstring 写着 vault「只读、永驻内存」。§2 的 P2 项已经指出这个措辞把
**永存**（never deleted）和 **RAM 驻留**混为一谈，并把措辞改诚实了。但改措辞不等于
补能力——事实仍然是：

> 月总结生成后，日记录确实一条没删。可一次进程重启，就把十年的事实全带走了。

那不是压缩删除，却是同一件事的另一种结果。而在手环上，进程被系统回收、设备重启是
**常态**，不是异常。所以工单 §2 的「底层原始事实必须永存」在默认配置下并不成立，
`TestConstitutionalRedLine` 那组测试证明的只是"同一进程生命周期内不删除"。

### 10.2 `VaultBackend`：注入式契约，不替 storage 层选型

新增 `VaultBackend` Protocol 与 `PyramidAggregator(vault_backend=...)`。**缺省为 None，
行为与既往逐位一致**（`test_default_configuration_has_no_persistence_and_is_unchanged`
专门守这条，否则 71 项测试的绿就成了假绿）。

契约只有三个方法：`put(event_id, payload)`、`items()`、`__len__()`。三条边界决策：

1. **只永存原始事实载荷**，派生量（`utc_time` / `base_weight` / `has_full_5d` / `xyz`）
   **不落盘**，重新装载时由 `_as_utc` 与 `_describe_event` 确定性重算。
   理由：落盘派生量等于落盘一份"可能过期的解释"，而 §93 严禁篡改历史要求历史只有
   一种权威形态，那就是原始事实本身。重算一致性由
   `test_rematerialization_from_restored_vault_is_lossless`（八个字段逐位相同）与
   `test_derived_quantities_recompute_identically_with_partial_5d`（含缺失 5D 的数据）固定。
2. **不持久化物化总结**。总结是可从事实完整重算的新观察层（§25 原文就说总结是
   "全新的观察层"），要永存的是事实，不是视图。视图落盘反而引入"事实与视图不一致"
   这个新的一致性问题。这是**有意决策**，由
   `test_views_are_deliberately_not_persisted` 固定：重启后 `summary_ids() == []`、
   `drill_down` 对旧 id 抛 unknown，但事实全在、视图可原地重建。
3. **隔离必须双向**。落盘取独立副本（后端不得与本进程内对象共享引用），水合也取
   独立副本（后端侧持有的 dict 被改动不得污染 vault）。任何一侧被改，永存记录都不动。

> **有意不做的事**：没有在聚合器内部接 sqlite。选存储引擎是 storage 层的架构决策
> （落盘时机、写入放大预算、事务边界），在聚合器里替它决定就是我上一轮批评过的
> "偷偷接盘"。本模块只定义契约，把选择权留给集成方。

### 10.3 跨重启生效的不只是"事实还在"

最重要的一条不是数据没丢，而是 **§93 的篡改检测跨重启生效**。摄入路径现在
**先水合再查重**：
...
如果不先水合，重启后的新实例看不到已永存的事实，一次改写历史的尝试就会被当成
**新事件放行**——`evidence conflict` 这道保护会在每次重启后失效一次。
`test_tamper_with_history_is_rejected_across_a_restart` 固定这条；变异 D1（去掉水合）
导致 2 项测试变红。

同时 `test_identical_replay_after_restart_is_idempotent_not_a_conflict` 守住另一侧：
同内容重放不是篡改，必须幂等通过，否则重启后正常回灌会全线报错。

### 10.4 开销实测（8784 条事实 / 闰年 × 每天 24 条）

| 路径 | 无后端 | 接持久后端 | 差值 |
|---|---|---|---|
| 物化 YEAR（含摄入落盘） | 138.97 ms | 167.02 ms | **+28.05 ms（+20%）** |
| YEAR → DAY 下钻 | 16.56 ms | 17.02 ms | +0.46 ms |
| 重启后水合 8784 条 | — | 39.07 ms | 每进程一次 |

- **45ms 红线在下钻路径上，未受影响**（17.02ms，余量 62%）。落盘成本落在物化路径，
  而物化不在红线内。
- +20% 的物化开销来自落盘前那次独立副本拷贝，约 **3.2 µs/条**。这是为 §93 的双向隔离
  付的钱，我认为该付；若后续压不下去，可以改成"后端自行负责序列化时拷贝"，但那会把
  隔离责任推给每个后端实现，风险更大。
- 水合 39ms/8784 条 = **4.4 µs/条**，每进程一次。十万级事实量约 0.5 秒——这就是
  契约里写明"规模再上一个量级应补 `get(event_id)` 支持惰性装载"的原因。

### 10.5 变异测试（6/6 全部被捕获）

| 变异 | 撤销的性质 | 结果 |
|---|---|---|
| D1 | 摄入前不水合（篡改历史被当新事件放行） | **2 failed** |
| D2 | 摄入后不落盘（永存变成一句口号） | **9 failed** |
| D3 | 落盘不取独立副本（与 vault 共享引用） | **1 failed** |
| D4 | 水合不取独立副本（与后端共享引用） | **1 failed** |
| D5 | 每次读都重新水合（O(n) 代价被放大到每次查询） | **1 failed** |
| D6 | 派生量直接落盘/不重算（把可能过期的解释当历史存下来） | **3 failed** |

### 10.6 剩余待决项（须由 storage 层承接）

- 生产级 `VaultBackend` 实现（sqlite / 文件 / KV）与落盘时机（写入即落盘 vs 批量）、
  写入放大预算、事务边界；
- 规模上到十万级以上时给契约补 `get(event_id)`，改为按 id 惰性装载；
- **注意 `raw_events_in_range` / `summaries_in_range` 的语义边界**（§9.5）：接上持久后端
  后，vault 成为跨进程的事实来源，"空结果"与"该区间确实没有事实"的不可区分性随之
  减轻，但只要存在尚未水合或尚未落盘的窗口，端侧仍不得把空列表渲染成
  「这一天什么都没发生」。这条在 G1 完全落地前必须保持。

## 附：一句话总结

聚合器主体是合格的，红线（总结不是压缩删除、原始事实永存、无损下钻、≤45ms）都真实成立
且经得起 17 个变异的检验。其中「原始事实永存」在默认配置下**原本并不成立**——
进程内 vault 一次重启就清零；现已由 `VaultBackend` 契约闭合（§10）。但它有一处**违宪级**缺陷：在完全没有 5D 数据时仍然告诉用户
