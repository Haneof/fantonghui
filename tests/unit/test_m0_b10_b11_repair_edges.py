...
from tests.unit.test_m0_gate_fifth_followup import obs, op, fake
...
def test_b10_unknown_dirty_type_is_protocol_rejection(tmp_path, declared, retry):
    ...
    with sqlite3.connect(store.db_path) as conn:
        for table in ('world_commits', 'object_revisions', 'operations', 'idempotency_records'):
            assert conn.execute(f'SELECT count(*) FROM {table}').fetchone()[0] == int(retry)
...
@pytest.mark.parametrize('key', [1, None, True, (1, 'a'), NumberKey.KEY, OpaqueKey.KEY, b'key'])
