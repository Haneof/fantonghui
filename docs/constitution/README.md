...
> **[`AIOS_Constitution_v3.0_FINAL.md`](./AIOS_Constitution_v3.0_FINAL.md)** —— 由下列全部现行法统文件**逐字合卷**而成的单一完整宪法（第零编立国宣言 / 第一~六编主宪法 / 第七编 ADJ 裁决集 / 第八编 R5 / 第九编 R6 / 第十编 Runtime Realignment 裁决 / 附录 A 候选 R4 / 附录 B 路线图），并在卷首给出法统层级与冲突裁决顺序。

日常阅读、Agent 派工与审计**一律以该合卷本为准**，无需再逐份拼读。合卷由 `scripts/build_constitution_final.py` 机械生成：它只重排标题层级与编次，不改写、不删减、不新增任何条款正文。**如需修法，先改下列原件，再重新执行该脚本重建合卷。**

## 1. 现行最高法统（合卷的来源原件，保留归档与溯源）
